...
当前版本已经继续补强“普通实时监控 + 噪音间隔监控 + 重点监控名单”的使用体验：

- “磁盘”页可以查看 `C:`、`D:` 等盘符的容量历史趋势。
- “目录树”页可以查看目录随快照变化的历史趋势。
- “噪音目录”页可以添加、删除和选择只做间隔快照的目录。
- “重点监控”页可以添加可疑目录，暂停、恢复、删除目标，并查看状态和剩余有效时间。
- “快速增长”页可以查看已经触发的增长提醒，以及最近目录树快照中增长较明显的线索。
- “设置”页可以明确选择记录程序启动方式：随 Windows 登录自动后台记录，或者手动打开记录程序。
- 后台记录模式带有托盘图标，可以通过右键菜单“退出 Disk History”正常停止后台程序。

这些功能仍然只记录本地元数据，不读取文件内容，不上传数据，也不自动删除文件。

## 2026-05-09 新监控模型

Disk History 现在按“盘符 + 目录树 + 实时事件”组织记录：

- 普通目录默认实时监控，同时定期记录最多 4 层目录树快照。
- 噪音目录默认不写实时事件，只做短间隔大小快照，例如 `Temp`、`.git`、`node_modules`、`.venv`、`build`、`dist`。
- 重点监控名单用于手动添加可疑目录，默认做更深层快照和独立日志，适合短期调查“这个目录为什么突然变大”。
- `C:`、`D:` 等盘符会记录整体容量、已用空间和可用空间。
...
- 调查报告导出：生成 Markdown 报告，并可隐藏常见私人路径。
...
默认会隐藏常见私人路径，例如把当前用户目录替换成 `{USERPROFILE}`。导出的报告仍可能包含目录名称和文件名，分享前建议快速检查。
